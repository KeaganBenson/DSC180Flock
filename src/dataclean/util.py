...
def temp_build_path_groupby(oa_collapsed_orders):
    assert "DESTINATION_3DIGIT_ZIP" in list(oa_collapsed_orders.columns)
    assert "ORIGIN_3DIGIT_ZIP" in list(oa_collapsed_orders.columns)
    
    assert 'LOG(RATE_USD)' in list(oa_collapsed_orders.columns)
    assert 'ORDER_OFFER_AMOUNT' in list(oa_collapsed_orders.columns)
    assert 'LOG(ORDER_OFFER_AMOUNT)' in list(oa_collapsed_orders.columns)
    assert 'REFERENCE_NUMBERS' in list(oa_collapsed_orders.columns)
    reference_numbers_column_name = "REFERENCE_NUMBERS"
    
    origin_zipcode_column = oa_collapsed_orders["ORIGIN_3DIGIT_ZIP"].astype(str)
    destination_zipcode_column = oa_collapsed_orders["DESTINATION_3DIGIT_ZIP"].astype(str)
    oa_collapsed_orders["PATH"] = origin_zipcode_column + "_TO_" + destination_zipcode_column
    
    temp_oa_collapsed_orders = oa_collapsed_orders.copy()
    ...
    temp_groupby = (
        ...
        .agg({
            "PATH_AVG_LOG(RATE_USD)": np.mean,
            "PATH_AVG_ORDER_OFFER_AMOUNT": np.mean,
            "PATH_AVG_LOG(ORDER_OFFER_AMOUNT)": np.mean,
        })
    )
    return temp_groupby

def add_path_columns(oa_collapsed_orders):
    '''
    ...
    assert 'REFERENCE_NUMBERS' in list(oa_collapsed_orders.columns)
    temp_groupby =  temp_build_path_groupby(oa_collapsed_orders)
    oa_collapsed_orders = oa_collapsed_orders.merge(temp_groupby,on=["PATH"])
    return oa_collapsed_orders
